...
static void ipc_ingest_task(void *arg) {
    ...
    while (1) {
        int rx_bytes = uart_read_bytes(UART_PORT_NUM, rx_buffer, sizeof(rx_buffer), pdMS_TO_TICKS(100));
                ...
                if (frame_idx > 0) {
                    ...
                    if (decoded_len >= IPC_FRAME_MIN_SIZE) {
                        ...
                        if (calc_crc == expected_crc) {
                            ...
                            ESP_LOGI(TAG, "Valid Frame RX! Type: 0x%02X, MAC: %02X:%02X:%02X:%02X:%02X:%02X, Seq: %d, RSSI: %d, Payload Size: %d",
                                     header->type,
                                     header->src_mac[0], header->src_mac[1], header->src_mac[2],
                                     header->src_mac[3], header->src_mac[4], header->src_mac[5],
                                     header->seq_num, header->rssi,
                                     (int)(decoded_len - sizeof(ipc_header_t) - 2));
                                     
                        } else {
                            ESP_LOGW(TAG, "CRC Error: Calc 0x%04X != Exp 0x%04X", calc_crc, expected_crc);
                        }
                    } else {
...
esp_err_t ipc_transport_init(void) {
    ...
    ESP_LOGI(TAG, "UART IPC Initialized on TX:%d RX:%d @ %d bps", UART_TX_PIN, UART_RX_PIN, UART_BAUD_RATE);
    return ESP_OK;
}

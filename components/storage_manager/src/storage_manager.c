...
{
    ESP_LOGI(TAG, "Initializing PMU LDO channel %d for MicroSD power...", CONFIG_SDCARD_LDO_CHANNEL);
    esp_ldo_channel_config_t ldo_cfg = {
        .chan_id = CONFIG_SDCARD_LDO_CHANNEL,
        .voltage_mv = 3300,
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to acquire LDO channel %d (err 0x%x)", CONFIG_SDCARD_LDO_CHANNEL, ret);
        return ret;
    }

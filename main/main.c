...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Telemetry buffer initialized (depth=%d)",
             CONFIG_TELEMETRY_QUEUE_SIZE);

    /* ── Phase 2: Ethernet (Core 0) ───────────────────────── */
    ESP_ERROR_CHECK(eth_manager_init());
    ESP_LOGI(TAG, "Ethernet manager initialized (MDC=%d, MDIO=%d)",
             CONFIG_ETH_MDC_GPIO, CONFIG_ETH_MDIO_GPIO);

    /* ── Phase 3: IPC Transport — UART from C6 (Core 1) ──── */
    ESP_ERROR_CHECK(ipc_transport_init());
    ESP_LOGI(TAG, "IPC transport initialized (TX=%d, RX=%d, baud=%d)",
    ...
    ESP_LOGI(TAG, "Cloud transport initialized (project=%s, topic=%s)",
             CONFIG_GCP_PROJECT_ID, CONFIG_GCP_PUB_SUB_TOPIC);
